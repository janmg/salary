...
public class WebController implements ErrorController {
    ...
    @Transactional
    @PostMapping("/newrate")
    public String newrate(@RequestParam Map<String,String> requestParams, Model model) {

        int persid = new Integer(requestParams.get("persid"));
        String rate=requestParams.get("rate");
        employeeRepository.setRateByPersid(persid, Double.parseDouble(rate));
        
        model.addAttribute("message", "rate for " +employeeRepository.findByPersid(persid).getName()+ " is now "+rate);
        model.addAttribute("employees", employeeRepository.findAll());
        model.addAttribute("content", "employees");  
        return "index";
    }
    ...
    @ResponseBody 
    public String download(Model model, HttpServletResponse response) throws IOException {
        byte[] file = sal.download();
